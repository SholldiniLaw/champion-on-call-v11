...
export function SuggestContractorClient() {
  ...
  if (submitted) return (
    <div className="max-w-lg mx-auto text-center py-16">
      ...
      <p className="text-navy-400 mb-6">Our team will review this contractor.</p>
  ...
  return (
    <div className="max-w-lg mx-auto">
      <div className="mb-8"><h1 className="text-2xl sm:text-3xl font-bold text-navy tracking-tight">Suggest a Contractor</h1><p className="text-sm text-navy-400 mt-1">Know a great contractor? Suggest them for our network.</p></div>
      <div className="card p-6 sm:p-8 space-y-5">
        <div className="flex items-center gap-3 mb-2"><div className="w-10 h-10 rounded-lg bg-navy-50 flex items-center justify-center"><UserPlus className="w-5 h-5 text-navy" /></div><div><h2 className="font-semibold text-navy text-sm">Contractor Information</h2><p className="text-xs text-navy-400">All suggested contractors go through our vetting process</p></div></div>
        ...
        <div><label className="label">Trade Type *</label><select className="input" value={form.tradeType} onChange={(e) => upd('tradeType', e.target.value)}><option value="WATER">Water Mitigation</option><option value="ROOF">Roofing</option><option value="STRUCTURAL">Structural</option></select></div>
        ...
        <button onClick={submit} disabled={isPending} className="btn-primary w-full gap-2">{isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <UserPlus className="w-4 h-4" />}{isPending ? 'Submitting...' : 'Submit Suggestion'}</button>
      </div>
    </div>
  );
}