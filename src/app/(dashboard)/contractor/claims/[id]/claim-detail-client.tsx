    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="card mb-4">
        <div className="flex items-center gap-2 mb-3">
          <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${isBlasting ? 'bg-amber-100 text-amber-700 animate-pulse' : isCheckedIn ? 'bg-emerald-100 text-emerald-700' : 'bg-violet-100 text-violet-700'}`}>{claim.status.replace('_', ' ')}</span>
      ...
      {isBlasting && !conflict && (
        <div className="card mb-4 border-l-4 border-l-crimson">
          <div className="flex items-center gap-2 mb-3"><Zap className="w-5 h-5 text-crimson" /><h2 className="font-semibold text-navy">Claim Available — First Come, First Served</h2></div>
      ...
      {conflict && (
        <div className="card mb-4 bg-crimson-50 border-crimson-200">
          <div className="flex items-center gap-3"><AlertTriangle className="w-6 h-6 text-crimson shrink-0" /><div><h2 className="font-semibold text-crimson">Claim Already Taken</h2><p className="text-sm text-crimson/70 mt-0.5">Another contractor accepted this claim before you.</p></div></div>
        </div>
      )}

      {isAssigned && (
        <div className="card mb-4 border-l-4 border-l-emerald-500">
          <div className="flex items-center gap-2 mb-3"><Navigation className="w-5 h-5 text-emerald-600" /><h2 className="font-semibold text-navy">On-Site Check-In</h2></div>
          ...
          <div className="flex items-center gap-3 mb-4 p-3 bg-amber-50 rounded-lg border border-amber-200">
            <button onClick={() => setDemoBypass(!demoBypass)} className="text-amber-600">{demoBypass ? <ToggleRight className="w-6 h-6" /> : <ToggleLeft className="w-6 h-6" />}</button>
            <div><p className="text-sm font-medium text-amber-700">Demo Mode {demoBypass ? 'ON' : 'OFF'}</p><p className="text-xs text-amber-600">Bypass geo requirement for demo</p></div>
          </div>
          {!demoBypass && (
            <>
              <button onClick={handleGeoCheck} disabled={geoLoading} className="btn-outline w-full mb-3 gap-2">{geoLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <MapPin className="w-4 h-4" />}{geoLoading ? 'Getting location...' : 'Check My Location'}</button>
              {geoStatus && (<div className={`p-3 rounded-lg mb-3 ${geoStatus.dist <= 100 ? 'bg-emerald-50 border border-emerald-200' : 'bg-crimson-50 border border-crimson-200'}`}><p className={`text-sm font-medium ${geoStatus.dist <= 100 ? 'text-emerald-700' : 'text-crimson'}`}>{geoStatus.dist <= 100 ? `✓ Within range — ${geoStatus.dist}m` : `✗ Too far — ${geoStatus.dist}m (max 100m)`}</p></div>)}
            </>
          )}
          <button onClick={handleCheckIn} disabled={checkingIn || (!demoBypass && (!geoStatus || geoStatus.dist > 100))} className="btn-primary w-full py-3 text-base gap-2">{checkingIn ? <Loader2 className="w-5 h-5 animate-spin" /> : <CheckCircle2 className="w-5 h-5" />}{checkingIn ? 'Checking in...' : 'Check In On-Site'}</button>
        </div>
      )}

      {isCheckedIn && (
        <div className="card mb-4 bg-emerald-50 border-emerald-200">
          <div className="flex items-center gap-3"><CheckCircle2 className="w-6 h-6 text-emerald-600 shrink-0" /><div><h2 className="font-semibold text-emerald-700">Checked In</h2><p className="text-sm text-emerald-600 mt-0.5">You have been verified on-site.</p></div></div>
        </div>
      )}
    </div>
  );
